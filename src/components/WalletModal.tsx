...
export default function WalletModal({ onClose }: WalletModalProps) {
  ...
  const fund = async () => {
    ...
    if (!Number.isFinite(value) || value < 100) {
      setMessage('Minimum funding amount is ₦100.');
      return;
    }
    setLoading(true);
    try {
    ...
    <div className="fixed inset-0 z-50 bg-black/40 flex items-end sm:items-center justify-center p-4">
      <div className="w-full max-w-md rounded-3xl bg-white shadow-xl overflow-hidden">
        <div className="flex items-center justify-between px-5 py-4 border-b">
          <div>
            <div className="flex items-center gap-2 font-bold text-gray-900"><Wallet className="w-5 h-5 text-rose-500" /> Wallet</div>
            <p className="text-sm text-gray-500 mt-1">Balance: <span className="font-semibold text-gray-900">₦{balance.toLocaleString()}</span></p>
          </div>
        ...
        <div className="p-5 space-y-4">
          ...
          <button onClick={fund} disabled={loading} className="w-full rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 text-white font-semibold py-3 disabled:opacity-60">
            {loading ? <Loader2 className="w-5 h-5 animate-spin mx-auto" /> : 'Continue to Flutterwave'}
          </button>
