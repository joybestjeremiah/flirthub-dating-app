...
const emptyEarnings: Earnings = { available_balance: 0, lifetime_earned: 0, lifetime_withdrawn: 0 };
...
export default function HostEarningsPage({ onBack }: { onBack: () => void }) {
  ...
  const load = async () => {
    ...
    setEarnings((e as Earnings | null) ?? emptyEarnings);
    ...
    <div className="min-h-screen bg-gradient-to-br from-rose-50 via-pink-50 to-orange-50">
      <header className="sticky top-0 z-30 bg-white/90 backdrop-blur border-b border-gray-100">
        <div className="max-w-md mx-auto px-4 py-3 flex items-center gap-3">
          <button onClick={onBack} className="p-2" type="button"><ArrowLeft className="w-5 h-5" /></button>
      ...
      <main className="max-w-md mx-auto px-4 py-5 space-y-4">
        <div className="grid grid-cols-3 gap-2">
          {[
            ...
            ['Earned', earnings.lifetime_earned],
            ['Withdrawn', earnings.lifetime_withdrawn],
          ].map(([label, value]) => (
        ...
        <div className="bg-white rounded-2xl border border-gray-100 p-5">
          ...
          <form onSubmit={requestWithdrawal} className="space-y-3">
            ...
            <button type="submit" disabled={busy} className="w-full py-3 rounded-xl bg-gradient-to-r from-rose-500 to-pink-600 text-white font-semibold disabled:opacity-50">
